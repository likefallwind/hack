...
class ClinicalApiTestCase(unittest.TestCase):
    ...
    def test_end_to_end_flow(self):
        ...
        local_asset = Path(self.tmpdir.name) / "frame1.txt"
        local_asset.write_text("demo-binary-content")
        add_asset_resp = self.client.post(
            "/assets",
            json={
                "asset_id": "A100-1",
                "visit_id": "V100-1",
                "asset_type": "image",
                "object_uri": str(local_asset),
            },
        )
        self.assertEqual(add_asset_resp.status_code, 201)
        download_resp = self.client.get("/assets/A100-1/download")
        self.assertEqual(download_resp.status_code, 200)
        _ = download_resp.get_data()
        download_resp.close()
    ...
    def test_validation_guardrails(self):
        ...
        self.assertEqual(self.client.get("/assets/not-exists/download").status_code, 404)
